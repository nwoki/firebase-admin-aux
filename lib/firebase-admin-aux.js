...
let FirebaseAdminAux = (function () {
    ...
    function initializeWithCache(projectId, clientEmail, privateKey, databaseUrl, enableCache = false) {
        return new Promise((resolve, reject) => {
            if (app == undefined) {
                redisCache = true;
                ...
                try {
                    ...
                    console.error("[FirabaseAdminAuth] %s", err);
                    reject(err);
                }
            } else {
                if (!redisCache) throw new Error("[FirabaseAdminAuth] already init without cache");
    ...
    function initialize(projectId, clientEmail, privateKey, databaseUrl, enableCache = false) {
        if (app == undefined) {
            ...
            try {
                if (redisCache) {
                    //Attention: not awaited!
                    return initializeWithCache(projectId, clientEmail, privateKey, databaseUrl);
                } else {
                    createFirebaseApp(projectId, clientEmail, privateKey, databaseUrl);
                    console.log(chalk.green("[FirabaseAdminAuth] Initialized"));
                }
