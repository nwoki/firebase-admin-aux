...
import { FirebaseAdminAux, CreateUserData, UpdateUserData } from '../src/lib/firebase-admin-aux';
...
let m_fbAdminAux: FirebaseAdminAux;
...
afterAll(async () => {
    await m_fbAdminAux.flushKeyValueDatabase();
...
describe('Test FirebaseAdminAux singleton veridicity', () => {
    it('Should fail due to init not having been called', async () => {
        expect(FirebaseAdminAux.instance()).toBe(undefined)
    });

    it('Should correctly return an instance once initialization is done', async () => {
        await m_fbAdminAux.init([
            { name: m_fbAdminConfigName, jsonCredentials: process.env.FIREBASE_TEST_JSON as string},
            { name: m_fbAdminConfigName, jsonCredentials: process.env.FIREBASE_TEST_JSON as string}
        ]);

        FirebaseAdminAux.setInstance(m_fbAdminAux);

        expect(FirebaseAdminAux.instance()).not.toBe(undefined);

        // Should not create two configurations with the same name
        expect(m_fbAdminAux.accounts().size).toBe(1);
    });
});
...
describe('Test the FirebaseAdminAux authentication middleware', () => {
    ...
    it('Should not pass due to malformed authentication header', async () => {
        ...
        await m_fbAdminAux.validateTokenMiddleware(mockRequest, mockResponse, nextFunction);
        expect(mockResponse.statusCode).toBe(StatusCodes.BAD_REQUEST);
    });
...
describe('Test the user operations', () => {
    ...
    const m_userUpdateData: UpdateUserData = {
        displayName: faker.lorem.word(),
        phoneNumber: faker.phone.number('+39##########'),
        emailVerified: true
    };

    let m_userFirebaseUid: string;
